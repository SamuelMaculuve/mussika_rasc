...
{
    public class ShopMussikaController : Controller
    {
        ...
        public ActionResult Index()
        {
            using (var db = new mussika_bdEntities())
            {
                ...
                List<categoria_artigo> categoriaArtigo_ = db.categoria_artigo.ToList();
                ViewBag.categorias = new SelectList(categoriaArtigo_, "id", "descricao");
        ...
        public List<ArtigoViewModel> ArtigoList()
        {
            ...
            using (var db = new mussika_bdEntities())
            {
                var artigos = db.artigoes.Where(p => p.Inactivo == false).OrderByDescending(P => P.id); ;
        ...
        public ActionResult filtrarAcomodacoes(ArtigoViewModel model1)
        {
            try
            {
                ...
                using (var db = new mussika_bdEntities())
                {
                    lista = db.artigoes.ToList();
                    if (model1.id_distrito != 0)
                    {
                        lista = lista.FindAll(a => a.id_distrito == model1.id_distrito).ToList();
                    }
                    if (model1.id_categoria_artigo != 0)
                    {
                        lista = lista.FindAll(a => a.id_categoria_artigo == model1.id_categoria_artigo).ToList();
                    }
                ...
                {
                    ArtigoViewModel acomo = new ArtigoViewModel()
                    {
                        atracao_internacional = (bool)model.atracao_internacional,
                        bairro = model.bairro,
                        data_cadastro = DateTime.Now,
                    ...
                    var nomeDit = procurarNomeDistrito((int)model.id_distrito);
                    var nomeCat = procurarNomeCategoria((int)model.id_categoria_artigo);

                    acomo.nomeCategoria = nomeCat;
        ...
        private string procurarNomeCategoria(int id_categoria_artigo)
        {
            using (var db = new mussika_bdEntities())
            {
                var tipo = db.categoria_artigo.Where(d => d.id == id_categoria_artigo).SingleOrDefault();
        ...
        public ActionResult SalvarAlteracoesArtigo(ArtigoViewModel model)
        {
            try
            {
                using (var db = new mussika_bdEntities())
                {
                    ...
                    artigo.preco_max = model.preco_max;
                    artigo.preco_min = model.preco_min;
